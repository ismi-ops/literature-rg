...
/* ── Top picks (highest-scored 4 from the latest batch) ─────────────────── */
const _latestDate = PAPERS.reduce((max, p) => (p.added||'') > max ? (p.added||'') : max, '');
const _latestBatch = PAPERS.filter(p => (p.added||'') === _latestDate)
  .slice().sort((a, b) => (b.score||0) - (a.score||0));
const TOP_PICK_IDS = new Set(_latestBatch.slice(0, 4).map(p => p.doi || p.title || ''));
function isTopPick(p) { return _latestDate && TOP_PICK_IDS.has(p.doi || p.title || ''); }
